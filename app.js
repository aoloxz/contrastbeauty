...
function generateSlots(date, duration=45){
  const range=weekdayRange(date);if(!range)return[];
  const slots=[];let t=range.start;
  while(true){const next=minutesAdd(t,duration);if(next>range.end)break;slots.push(t);t=next;}
  return slots;
}
...
function Confirm(){
  ...
  return `<section class='card confirm-card'>
    ...
    <button class="btn primary big" id="confirm-booking">Confirmă programarea</button>
  </section>`;
}

/* Alte pagini simple */
function Card(u){return `<section class='card'><h2>Card membru</h2><p>${u?.visits||0} vizite</p></section>`}
...
function bindEvents(u){
  ...
  $$('#daylist .day')?.forEach(d=>d.onclick=()=>{bookingDraft.dateISO=d.dataset.date;navTo('appointment')});
  $$('.slot')?.forEach(s=>s.onclick=()=>{bookingDraft.time=s.textContent;navTo('confirm')});

  $('#confirm-booking')?.addEventListener('click',async()=>{alert('Programare confirmată!');navTo('home')});

  $('#login-btn')?.addEventListener('click',async()=>{const email=$('#login-email').value,pass=$('#login-pass').value;
  ...
  $('#signup-btn')?.addEventListener('click',async()=>{const name=$('#su-name').value,email=$('#su-email').value,pass=$('#su-pass').value;
...
/* Splash intro */
window.addEventListener('load',()=>{setTimeout(()=>$('#splash').classList.add('run'),900);
