...
const CURRENT_KEY = 'cb_current_user';
const REWARD_STAMPS = 5;
...
const signupApi = d => api('signup', 'POST', d),
  ...
  updateBookingApi = (id, status) =>
    api('updateBooking', 'POST', { id, status });
...
function minutesAdd(hm, plus) {
  const [h, m] = hm.split(':').map(Number);
  const t = h * 60 + m + plus;
  return toTime(Math.floor(t / 60), t % 60);
}
...
function Services() {
  return `<section class='card'>
    ...
    <div class="gender-select">
      <button class="btn gender-btn" data-gender="barbat">Bărbat</button>
      <button class="btn gender-btn" data-gender="femeie">Femeie</button>
    </div>
    <div id="services-list"></div>
  </section>`;
}

function renderServicesList(gender) {
  const male = [
    { name: 'Tuns clasic', dur: 45, price: '70' },
    { name: 'Fade + Styling', dur: 60, price: '90' },
    { name: 'Barbă & Contur', dur: 30, price: '50' }
  ];
  const female = [
    { name: 'Spălat + styling', dur: 30, price: '60' },
    { name: 'Tuns & coafat', dur: 60, price: '120' }
  ];
  const items = gender === 'barbat' ? male : female;
  return `<div class="services-list">${items
    .map(
      s => `
    <div class="service-card">
      <div>
        <strong>${s.name}</strong><br><span>${s.dur} min • ${s.price} lei</span>
      </div>
...
function Appointment() {
  ...
  return `<section class='card'>
    <div class="step-header">
      <button id="prev-week" class="nav-small">◀</button>
      <strong>Alege ora</strong>
...
function Confirm() {
  if (!bookingDraft)
    return `<section class='card'><h2>Confirmă</h2><p>N-ai selectat un slot.</p></section>`;
  ...
  return `<section class='card'>
    ...
    <p>${bookingDraft.service} – ${bookingDraft.duration} min – ${bookingDraft.price} lei</p>
    <button class="btn primary full-width" id="confirm-booking">Confirmă</button>
  </section>`;
}

function Card(u) {
  if (!u)
    return `<section class='card'><h2>Card</h2><p>Autentifică-te.</p></section>`;
...
function bindEvents(u) {
  ...
  $$('.gender-btn').forEach(btn => {
    btn.onclick = () => {
      $('#services-list').innerHTML = renderServicesList(btn.dataset.gender);
      $$('.book-btn').forEach(b => {
        b.onclick = () => {
          bookingDraft = {
  ...
  $('#confirm-booking')?.addEventListener('click', async () => {
    const body = {
      ...
      date: new Date(bookingDraft.dateISO).toISOString().slice(0, 10),
  ...
  $('#signup-btn')?.addEventListener('click', async () => {
    ...
    try {
      const user = await signupApi({ name, email, password: pass });
      setCurrentUser(user.email);
      navTo('home');
    } catch (e) {
      alert('Eroare signup');
  ...
  if (route === 'admin') {
    $$('.admin-booking [data-action]').forEach(btn => {
      btn.onclick = async () => {
        await updateBookingApi(
          btn.closest('.admin-booking').dataset.id,
          btn.dataset.action === 'cancel' ? 'canceled' : 'done'
        );
        alert('Actualizat');
        navTo('admin');
      };
...
window.addEventListener('load', () => {
  ...
  setTimeout(() => {
    ...
    document
      .querySelectorAll('.site-header,.site-footer,#app')
